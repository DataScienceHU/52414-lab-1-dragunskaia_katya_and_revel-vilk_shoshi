...
#Analysis of the data on the 31.3.2020
```{r}
...
```
### b
In order not to lose essentail data we decided to use the countries abbreviations from the original eco_data and to add abbreviations to the corona data-set. 
```{r}
codes_corona = c()
k = 1
for (i in as.character(corona$Country.Region)){
  codes_corona[k] <- as.character(data_con$ISO.3166.1.Alpha.3.Codes[which(i == as.character(data_con$Country.Region))][1])
  k<- k+1
}
corona = corona %>% mutate(S_country = codes_corona)
eco_code = eco_data %>% select(S_country, feature, Y2018V) %>% filter(as.character(feature) != "") %>% spread(feature, Y2018V)  %>%  select(S_country, GDP = `GDP (current US$)`, pop_total = `Population, total`, pop_city_ratio = `Population in the largest city (% of urban population)`, pop65 = `Population ages 65 and above (% of total population)`) 
x = setdiff(corona$S_country, eco_code$S_country)
y= c("Vatican", "Kosovo", "Taiwan", "Westren Sahara")
print(paste("Missing data for ", c(x[2:5]),"-", y[1:4]))

```
After we showed we will we kept most of the data we can use inner join to join eco_code and corona by S_country
```{r}
corona_eco = corona %>% inner_join(eco_code, by = 'S_country') 
...
#2
corona_eco_march = corona_eco %>% filter(Date == "2020-03-31") %>% arrange(desc(norm_cases)) %>% top_n( 10, norm_cases) %>% select(Country.Region, norm_cases, norm_deaths, norm_recovered)
...
{cases.plot = barplot(cases_norm.t[1,], ylab = "Corona cases per 10000 population", xlab= "Countries", main = "Top 10 countries with normalized cases in 31 March 2020", width= 0.5,  cex.axis = 0.75, cex.lab= 1, las= 1, col = "red", ylim= c(0,1000))
others.plot = barplot(cases_norm.t[2:3,], width= 0.5, cex.axis = 0.75, cex.lab= 1, las= 1, col=c("green","dark blue"), add = T)
legend(4.5, 800, c("sick", "dead", "recovered"), fill= rainbow(3))}
```
## The scale of normalized cases is significanctly lower and the top 10 countries are different. 

### d
First we create data frame to make work easily. Then we build plot using basic graph function.
```{r}
corona_eco_plot = corona_eco %>% filter(pop65 >= 15) %>% filter(Date == "2020-03-31") %>% select(Country.Region, norm_cases, norm_deaths, norm_recovered, pop65)
print(corona_eco_plot)
#making the plot#
par(mar=c(4, 4, 4, 4) + 0.1)
plot(corona_eco_plot$pop65, corona_eco_plot$norm_cases, 
      xlab= "% population 65 years or above", 
      ylab= "Normalized confirmed cases", 
      main= "Corona cases and death in 31 March 2020",
      pch=19, 
     col="green", 
     ylim=c(min(corona_eco_plot$norm_cases), max(corona_eco_plot$norm_cases)),
     xlim=c(min(corona_eco_plot$pop65), max(corona_eco_plot$pop65)+5))
par(new=TRUE)
plot(corona_eco_plot$pop65, corona_eco_plot$norm_deaths, pch= 19,  xlim=c(min(corona_eco_plot$pop65), max(corona_eco_plot$pop65)+5), axes=F, xlab="", ylab="", col = ifelse(corona_eco_plot$norm_deaths > 15 | corona_eco_plot$pop65 > 24, "red", "black"))
axis(4)
mtext("Normalized death cases", line=3, side=4)
legend("topright", c("confirmed", "dead"), box.lty = 0, lty=1, pch=19, col=c("green", "black"), inset=0.05)
text(corona_eco_plot$pop65[which(corona_eco_plot$pop65 > 24)], 
     corona_eco_plot$norm_cases[which(corona_eco_plot$pop65 > 24)], 
     labels = corona_eco_plot$Country.Region[which(corona_eco_plot$pop65 > 24)])
text(corona_eco_plot$pop65[which(corona_eco_plot$norm_deaths > 15)], 
     corona_eco_plot$norm_deaths[which(corona_eco_plot$norm_deaths > 15)], 
     labels = corona_eco_plot$Country.Region[which(corona_eco_plot$norm_deaths > 15)])
...
YOUR SOLUTION HERE.
# Research question: Is the risk of death from Corona related to the health expenditure per capita (current US$) ? 
#Data is taken from the world bank, updated for 2017 (no 2018 or 2019 exists). 
#loading the health expenditure data`:
```{r}

CHE_data = read.csv(url("https://raw.githubusercontent.com/DataScienceHU/52414-lab-1-dragunskaia_katya_and_revel-vilk_shoshi/master/API_SH.XPD.CHEX.PC.CD_DS2_en_csv_v2_1001395.csv"))
CHE_data = CHE_data %>%  select(S_country= Country.Code, CHE2017 = X2017)

```
#creating a dataset of risk of death in the last day of corona_eco data. We limitted the database  for a risk of death higher than 1% and countries with more than 1000 cases.

```{r}
corona_eco$Date  = as.Date(corona_eco$Date)
latest = max(corona_eco$Date)
death_risk = corona_eco %>% filter(Date == latest) %>% mutate(risk= deaths/cases*100) %>%  mutate(GDP_capita= GDP/pop_total) %>% filter(risk > 1) %>% filter(cases >1000) %>%  select(Country.Region, S_country, Date, risk, cases, deaths, GDP_capita, pop_total, pop_city_ratio, pop65)
```
# joining the risk_corona and CHE data

```{r}
Q5 = death_risk %>% left_join(CHE_data, by = 'S_country') %>% mutate(logCHE = log(CHE2017))
```

#creating the plot 
```{r}

lm_CHE <-lm(formula = as.vector(Q5$logCHE) ~ as.vector(Q5$risk))
corr = cor.test(x= Q5$risk, y= Q5$logCHE)
plot(x= Q5$risk, y= Q5$logCHE, main= "Corona deaths and health expendure", xlab = "Risk of death from Corona", ylab = "Health expendure per capita (log)", abline(lm_CHE, col = "red"))
print("Increased health expendure per capita in 2017 is weakly correlated with higher risk of death from Corona")
print(corr)
